...
def exec(lat,lng):
    try:
        url='http://dataservice.accuweather.com/locations/v1/cities/geoposition/search?apikey='+key.accuKey+'&q='+str(lat)+','+str(lng)
        locKey=json.loads(requests.get(url).text)['Key']
        url2='http://dataservice.accuweather.com/currentconditions/v1/'+locKey+'?apikey='+key.accuKey+'&details=true'
